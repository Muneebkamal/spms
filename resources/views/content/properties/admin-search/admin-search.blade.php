...
<div class="card" style="overflow-x: auto">
        ...
        <form id="myForm" method="GET">
            <div class="row">
                <div class="col-md-12 d-flex justify-content-between">
                    <div class="rent">
                        <input type="checkbox" id="types1" name="type[]" value="Rent 放租"
                            class="types1 form-check-input">
                        <label>Rent 放租</label>
                    </div>
                    <div class="sales">
                        <input type="checkbox" id="types1" name="type[]" value="Sales 放售"
                            class="types1 form-check-input">
            ...
            <div class="col-md-12 mt-2">
                ...
                <select id="types" name="type[]" class="form-select select2" multiple>
                    ...
                    <option value="{{$type->name}}"
                        {{ !empty($_GET['type']) && $_GET['type'] == $type->name ? 'selected':''}}>{{$type->name}}
                    </option>
                    @endif
                    @endforeach
                </select>
    ...
    <div class="row mt-3 d-flex">
        <div class="col-md-6">
            <div id="buttons" class="d-flex">
                <button class="btn btn-success me-2 saveCustomer_processing d-none" disabled>Processing ...</button>
                <button class="btn btn-success me-2 saveCustomers_sve_btn">{{__('Search')}}</button>
    ...
    <div class="row py-4" id="price-size">
        ...
        <div class="col-md-12 mt-3 d-flex">
            <button class="btn btn-success me-2" type="submit">{{__('Search')}}</button>
            <button class="btn btn-danger">{{__('Clear')}}</button>
        </div>
    </div>
    </form>
...
<div id="resultFound" class="border-top d-none p-4 d-flex justify-content-between">
    <h4 class="mb-0 pb-0" id="search_cnt_tag" style="display:none">Results Found:<strong id="search_count"></strong>
    </h4>
    <button class="toggleTableView btn h-50 btn-outline-primary" style="display: none">Go Back</button>
    <button class="toggleTableView btn h-50 btn-outline-primary">See Table</button>
</div>
<div id="fetchProperty" class="px-4"></div>
<div id="fetchPropertyTable" style="display: none" class="px-4">
    <table class="table border" id="resultsTable">
        <thead>
            <tr>
